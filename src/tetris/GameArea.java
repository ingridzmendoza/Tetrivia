...
import tetrisblocks.*; //imports all classes from the package
...
public class GameArea extends JPanel{
    ...
    private TetrisBlock[] blocks; //stores all available shapes to spawn them at random
    ...
    public int clearLines(){
        boolean lineFilled;
        int linesCleared = 0; //score counter
    ...
    private void drawBlock(Graphics g){
        ...
        for(int row = 0; row < h; row++){
            for(int col = 0; col < w; col++){
                if(shape[row][col] == 1){ //calls getShape function
